...
```ts
  ...
  15 | 
  16 |     let poption:Locator = page.locator('ul.context-menu-item > span');
  17 | 
> 18 |     await expect( poption).toBeDisabled();
     |                            ^ Error: expect(locator).toBeDisabled() failed
  19 | 
  20 |     await page.locator('h1').click({button: 'right'});
  21 |   //  await page.waitForTimeout(5000);
  22 |     await expect( page.locator("ul.context-menu-list")).not.toBeVisible();
  23 | });
```