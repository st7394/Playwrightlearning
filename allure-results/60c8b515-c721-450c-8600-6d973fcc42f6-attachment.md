...
- Name: scratch.spec.ts >> inspect paste
- Location: tests\scratch.spec.ts:2:5

# Error details

```
Error: locator.evaluate: Error: strict mode violation: getByText('Paste', { exact: true }) resolved to 2 elements:
    1) <em>Paste</em> aka getByRole('emphasis').filter({ hasText: 'Paste' })
    2) <span>Paste</span> aka getByRole('button', { name: 'Paste ⌘V' })

Call log:
  - waiting for getByText('Paste', { exact: true })
...
```ts
  ...
  2 | test('inspect paste', async ({ page }) => {
  3 |     await page.goto('https://app.thetestingacademy.com/playwright/widgets/context-menu');
  4 |     await page.locator('span.context-menu-one').first().click({ button: 'right' });
> 5 |     const pasteHtml = await page.getByText('Paste', { exact: true }).evaluate(el => el.parentElement?.outerHTML);
    |                                                                      ^ Error: locator.evaluate: Error: strict mode violation: getByText('Paste', { exact: true }) resolved to 2 elements:
  6 |     console.log("PASTE_HTML:", pasteHtml);
  7 | });
  8 | 
```