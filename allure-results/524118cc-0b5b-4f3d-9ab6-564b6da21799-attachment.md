...
Call log:
  - waiting for getByTestId('test-id-Taxi')
    - locator resolved to <a href="#" role="menuitem" class="submenu-item" data-testid="test-id-Taxi">…</a>
...
> 8  |         await page.getByTestId('test-id-Taxi').click();
     |                                                ^ Error: locator.click: Test timeout of 30000ms exceeded.
  9  |         await page.waitForTimeout(5000);
  10 | 
  11 |         let response: Locator = page.locator('#output');
  12 |         await expect(response).toBeVisible();
  13 |         expect(response).toHaveText('Selected option Taxi');
  14 | 
  15 | 
  16 |     });
  17 | });
