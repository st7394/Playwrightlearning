...
```
Error: locator.isDisabled: Error: strict mode violation: getByText('Paste', { exact: true }) resolved to 2 elements:
    1) <em>Paste</em> aka getByRole('emphasis').filter({ hasText: 'Paste' })
    2) <span>Paste</span> aka getByTestId('ctx-menu').getByText('Paste')
...
```ts
  ...
  15 | 
  16 |     let poption:Locator = await page.getByText('Paste', { exact: true });
  17 | 
> 18 |     expect(await poption.isDisabled()).toBe(true);
     |                          ^ Error: locator.isDisabled: Error: strict mode violation: getByText('Paste', { exact: true }) resolved to 2 elements:
  19 | 
  20 |     await page.locator('h1').click({button: 'right'});
  21 |     await page.waitForTimeout(5000);
  22 |     expect(await page.locator("context-menu-item")).not.toBeVisible();
  23 | });
```